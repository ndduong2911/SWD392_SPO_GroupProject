...
namespace SharingPictureOnline.Services
{
    public class NotificationService : INotificationService
    {
        ...
        // Kết hợp Inject cả Repository, StateService và DBContext vào Constructor
        ...
        #region Logic từ nhánh Follow_Unfollow_branch (Tạo thông báo)
        ...
        #region Logic từ nhánh Handle_Report (Lấy dữ liệu & Đọc thông báo)
        ...
        {
            // Tối ưu hóa hàm MarkAllAsRead để dùng luôn DbContext của nhánh Handle_Report
        ...
        private async Task<Dictionary<Guid, TargetSubject>> ResolveTargetSubjectsAsync(IEnumerable<Report> reports)
        {
            ...
            var profileIds = list.Where(r => r.TargetType == "PROFILE").Select(r => r.TargetId).Distinct().ToList();
            ...
            var profiles = profileIds.Count == 0
                ? new Dictionary<Guid, (Guid UserId, string Username)>()
                : await _context.UserProfiles.AsNoTracking()
                    .Include(p => p.User)
                    .Where(p => profileIds.Contains(p.ProfileId))
                    .ToDictionaryAsync(
                        p => p.ProfileId,
                        p => (p.UserId, p.User.Username));
            ...
            foreach (var report in list)
            {
                result[report.ReportId] = report.TargetType switch
                {
                    ...
                    "PROFILE" => BuildProfileSubject(report.TargetId, profiles),
        ...
        private static TargetSubject BuildProfileSubject(
            Guid profileId,
            IReadOnlyDictionary<Guid, (Guid UserId, string Username)> profiles)
        {
            if (profiles.TryGetValue(profileId, out var profile) && !string.IsNullOrWhiteSpace(profile.Username))
                return new TargetSubject("Trang cá nhân", profile.Username, $"/profile/{Uri.EscapeDataString(profile.Username)}");
            return new TargetSubject("Trang cá nhân", "trang cá nhân đã báo cáo", null);
        }
